...
public class PasswordMasker implements LogMasker {
    private final List<String> keywords = Arrays.asList("password", "rootPassword","zstackPassword", "mysqlRootPassword", "mysqlNewRootPassword", "newPassword", "mysqlPassword","adminPassword");
    ...
    private boolean isPasswordStart(Character character, StringBuilder builder, int pos) {
        return keywords.stream().anyMatch(k -> k.charAt(0) == character)
                || ('<' == character && keywords.stream().anyMatch(k -> k.charAt(0) == builder.charAt(pos+1)));
    }
